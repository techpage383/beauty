    ...
    <div>
        ...
        {stats.map(s => (
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <p className="text-base font-semibold opacity-80">{s.label}</p>
              <FontAwesomeIcon icon={s.icon} className="w-5 h-5 opacity-70" />
            </div>
            <p className="text-5xl font-bold">{s.value.toLocaleString()}</p>
          </Link>
        ))}
      </div>
      ...
      {(pending ?? 0) > 0 && (
        <div className="mt-8 bg-yellow-50 border border-yellow-200 rounded-2xl p-5 flex items-center justify-between">
          <div>
            <p className="text-lg font-semibold text-yellow-800">審査待ちの口コミがあります</p>
            <p className="text-lg text-yellow-600 mt-0.5">{pending}件の口コミが審査を待っています</p>
          </div>
          <Link
            href="/admin/reviews?status=pending"
            className="bg-yellow-500 text-white px-4 py-2 rounded-xl text-lg font-semibold hover:bg-yellow-600 transition-colors shrink-0 flex items-center gap-2"
          >
            確認する
