              ...
              <tr key={t.id} className="hover:bg-gray-50 transition">
                ...
                <td className="px-4 py-3">
                  <button onClick={() => togglePublish(t)}
                    className={`text-xs px-2 py-0.5 rounded-full ${t.is_published ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                    {t.is_published ? '公開中' : '非公開'}
                  </button>
                </td>
                <td className="px-4 py-3">
                  <button onClick={() => remove(t.id)} className="text-xs text-red-400 hover:text-red-600 transition">削除</button>
                </td>
              </tr>
            ))}
          </tbody>
