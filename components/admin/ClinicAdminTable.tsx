  ...
  return (
    <div>
      <button
        ...
        className="mb-4 bg-gray-900 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-gray-700 transition"
      >
        + クリニックを追加
      </button>
      ...
      {adding && (
        <div className="bg-white border border-gray-100 rounded-2xl p-5 mb-4 space-y-3">
          ...
          <button onClick={add} className="bg-brand-600 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-brand-700 transition">
            追加する
          </button>
        </div>
      )}
              ...
              <tr key={c.id} className="hover:bg-gray-50 transition">
                ...
                <td className="px-4 py-3">
                  <button onClick={() => togglePublish(c)}
                    className={`text-xs px-2 py-0.5 rounded-full ${c.is_published ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                    {c.is_published ? '公開中' : '非公開'}
                  </button>
                </td>
                <td className="px-4 py-3">
                  <button onClick={() => remove(c.id)} className="text-xs text-red-400 hover:text-red-600 transition">削除</button>
                </td>
              </tr>
            ))}
          </tbody>
